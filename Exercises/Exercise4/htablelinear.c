...
void insertTable(struct htable *h, char word[]) {
   ...
   */
  int i = 0;
  while (strcmp(h->entries[(hashval + i * i) % TABLE_SIZE], "") != 0) {
    i++;
  }
  strcpy(h->entries[(hashval + i * i) % TABLE_SIZE], word);
}
...
int searchTable(struct htable *h, char word[]) {
  ...
  int i = 0;
  while (strcmp(h->entries[(hashval + i * i) % TABLE_SIZE], "") != 0 &&
         strcmp(h->entries[(hashval + i * i) % TABLE_SIZE], word) != 0) {
    i++;
  }

  // The word was in the table.
  if (strcmp(h->entries[(hashval + i * i) % TABLE_SIZE], word) == 0)
    return 1;
...
void deleteTable(struct htable *h, char word[]) {
  ...
  if (strcmp(h->entries[(hashval + i * i) % TABLE_SIZE], word) == 0) {
    strcpy(h->entries[(hashval + i * i) % TABLE_SIZE], "");
  }
