...
struct tree_node *search(struct tree_node *current_ptr, int value);
...
struct tree_node *search(struct tree_node *current_ptr, int value) {
  ...
  if (current_ptr != NULL) {
    ...
    if (value < current_ptr->data)
      return search(current_ptr->left, value);

    // Or...search to the right.
    else
      return search(current_ptr->right, value);
...
struct tree_node *delete(struct tree_node *root, int value) {
  ...
  delnode = search(root, value); // Get a pointer to the node to delete.
